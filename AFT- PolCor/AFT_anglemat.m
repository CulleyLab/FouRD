function [anglemat,exccentricitymat,pc,pr,vc2,ur2,excc] = AFT_anglemat(im, parameters)
...
exccentricitymat = zeros(numRows,numCols) * NaN;
...
for i = 1:numRows
            ...
            if mean(window(:)) > checkpoint
                ...
                if sum(window_fft(:)) ~= 0 
                    ...
                    if eccentricity > eccentricity_threshold
                        anglemat(i,j) = theta;
                        exccentricitymat(i,j) = eccentricity;
                    else
                        anglemat(i,j) = NaN;
                        exccentricitymat(i,j) = Nan;
                    end
    ...
    if (i < (length(grid_row)+1)) && (parameters.parproc == 0)
